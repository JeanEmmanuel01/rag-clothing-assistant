...
# Directory holding product images, named "<product_id>.jpg"
IMAGES_DIR = Path(os.environ.get("PRODUCT_IMAGES_DIR", "dataset/images"))
...
# Legacy workaround for calling the LLM API through a proxy; do not reproduce this
# pattern elsewhere, and it should be removed for real (non-proxied) usage.
...
# Kept because rag_clothing_assistant.ipynb calls this before importing flask_app and
# weaviate_server, to free the ports they bind to on re-run.
...
def generate_with_single_input(prompt: str, role: str = 'user', top_p: float = None, 
    ...
    if (not together_api_key) and ('TOGETHER_API_KEY' not in os.environ):
        url = os.path.join(os.environ.get("TOGETHER_BASE_URL", "https://api.together.xyz/"), 'v1/chat/completions')
        # Legacy proxy workaround: TLS verification disabled. Do not reproduce this
        # pattern elsewhere; it should be removed for real usage.
...
class ChatWidget:
    ...
    def load_image(self, id: str):
        ...
        """
        image_path = IMAGES_DIR / f"{id}.jpg"
